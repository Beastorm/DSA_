...
**Input:** asteroids = [2, -2]  
**Output:** []  
**Explanation:** The asteroid with size 2 and the one with size -2 collide, exploding into each other.
...
2. Loop through each asteroid in the input array:
   ...
   - If the asteroid is moving left (negative), check for collisions with the top elements of the stack:
...
public:
    ...
    vector<int> asteroidCollision(vector<int> &asteroids){
        ...
        for(int i=0; i < n; i++) {
            ...
            // Push the asteroid in the stack if a right-moving asteroid is seen
            if(asteroids[i] > 0) {
            ...
            // Else if the asteroid is moving right, perform the collisions 
                ...
                // If there is a right-moving asteroid that is of the same size
...
int main() {
    ...
    // Creating an instance ofthe  Solution class
    Solution sol; 
