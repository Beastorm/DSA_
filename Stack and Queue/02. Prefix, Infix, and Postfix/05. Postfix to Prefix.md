...
3. _For each character:_  
   - If it is an operand, push it onto the stack.
   - If it is an operator, pop two operands, combine them with the operator in prefix order, and push the result back onto the stack.
...
string postfixToPrefix(string postfix) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (isalnum(c)) {
            ...
            // Form the new prefix expression and push back to the stack
            s.push(c + op1 + op2);
        }
    }
...
- **Time Complexity:** O(n)  
  As we traverse the expression only once.

- **Space Complexity:** O(n)  
  for the stack to store operands and intermediate results.
