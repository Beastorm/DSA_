## Infix to Prefix

**Problem Statement:**  
Given an infix expression, your task is to convert the given infix expression to a prefix expression.

---

>### Examples:

**Example 1:**  
**Input:**    
`x + y * z / w + u`   

**Output:**    
`++x/*yzwu`  

**Explanation:**      
The infix expression is converted to prefix form as `"++x/*yzwu"` by applying the rules of infix to prefix conversion.

---

**Example 2:**  
**Input:**     
`a + b`  

**Output:**    
`+ab`  

**Explanation:**    
The infix expression `"a + b"` is converted to prefix form as `"+ab"`.

---

## Approach

### Algorithm:

**What is an Infix Expression?**  
- The traditional method of writing mathematical expressions, where the operator is placed between two operands (e.g., A+B or (A*B)/Q).
- Infix expressions are easy for humans but hard for computers to parse due to operator precedence, associativity rules, and parentheses.

**What is a Prefix Expression?**  
- In a prefix expression, the operator is placed before the operands (e.g., +AB).
- Prefix expressions are also called Polish notation.

**Approach to Convert Infix Expression to Prefix:**

1. Reverse the given infix expression.
2. Scan the expression from left to right.
3. When an operand is encountered, print it directly.
4. If the operator is encountered and the stack is empty, push the operator into the stack.
5. If the incoming operator has higher precedence than the top of the stack, push it into the stack.
6. If the incoming operator has the same precedence as the top of the stack, push it into the stack.
7. If the incoming operator has lower precedence than the top of the stack, pop and print the top of the stack. Then, test the incoming operator against the top of the stack again and continue popping operators from the stack until it finds an operator with lower or the same precedence.
8. If the incoming operator has the same precedence as the top of the stack and the incoming operator is `^`, pop the top of the stack until the condition is met. If not, push the `^` operator.
9. If the operator is `)`, push it into the stack.
10. If the operator is `(`, pop all operators from the stack until a `)` is encountered.
11. If the top of the stack is `)`, push the operator onto the stack.
12. At the end of the expression, pop and print all remaining operators from the stack.
13. Finally, `reverse` the output and print it as the prefix expression.


#### Why do we reverse the infix expression initially?
Prefix notation (Polish notation) requires operators to appear before their operands.
The standard infix-to-postfix algorithm works left-to-right, but prefix is naturally right-to-left.
By reversing the infix expression, we can use a similar stack-based algorithm as infix-to-postfix, but process the expression in a way that produces prefix order.   

>**Point:**  
For `^`, you want to keep the stack top `^` if it has the same precedence, so the rightmost `^` is evaluated first (right-associative).
For other operators, you want to pop the stack top if it has the same precedence, so the leftmost operator is evaluated first (left-associative).


>**In short:**  
- Popping an operator from the stack means it will be evaluated before the current operator, according to the precedence and associativity rules.
- If the current operator has lower precedence than the stack top, you pop the higher precedence operator(s) from the stack and add them to the output, because they must be evaluated first.



### C++ Code:
``` cpp
#include <bits/stdc++.h>
using namespace std;

// Function to check if a character is an operator
bool isOperator(char c) {
    return (!isalpha(c) && !isdigit(c));  // If the character is neither alphabetic nor numeric, it's an operator
}

// Function to return the precedence of operators
int getPriority(char C) {
    if (C == '-' || C == '+')  // Addition and subtraction have lowest precedence
        return 1;
    else if (C == '*' || C == '/')  // Multiplication and division have higher precedence
        return 2;
    else if (C == '^')  // Exponent operator has highest precedence
        return 3;
    return 0;
}

// Function to convert an infix expression to a postfix expression
string infixToPostfix(string infix) {
    infix = '(' + infix + ')';  // Add parentheses to handle edge cases
    int l = infix.size();
    stack<char> char_stack;  // Stack to store operators
    string output;  // String to store the resulting postfix expression

    for (int i = 0; i < l; i++) {

        // If the scanned character is an operand, add it to the output
        if (isalpha(infix[i]) || isdigit(infix[i]))
            output += infix[i];

        // If the scanned character is ‘(’, push it to the stack
        else if (infix[i] == '(')
            char_stack.push('(');

        // If the scanned character is ‘)’, pop and output from the stack until an ‘(‘ is encountered
        else if (infix[i] == ')') {
            while (char_stack.top() != '(') {
                output += char_stack.top();
                char_stack.pop();
            }
            char_stack.pop();  // Remove '(' from the stack
        }

        // If an operator is found
        else {
            // Handle associativity for right-associative '^' and left-associative others
            while (!char_stack.empty() && isOperator(char_stack.top()) &&
                  ((infix[i] != '^' && getPriority(infix[i]) <= getPriority(char_stack.top())) ||
                   (infix[i] == '^' && getPriority(infix[i]) < getPriority(char_stack.top())))) {
                output += char_stack.top();
                char_stack.pop();
            }
            // Push current operator on stack
            char_stack.push(infix[i]);
        }
    }
    
    // Pop all remaining elements from the stack
    while (!char_stack.empty()) {
        output += char_stack.top();
        char_stack.pop();
    }
    return output;  // Return the postfix expression
}

// Function to convert an infix expression to a prefix expression
string infixToPrefix(string infix) {
    int l = infix.size();

    // We reverse the infix expression at the start so we can use the postfix conversion logic 
    // (which is left-to-right) to generate a prefix expression (which is right-to-left).
    // Reverse the infix expression
    reverse(infix.begin(), infix.end());

    // Replace '(' with ')' and vice versa
    for (int i = 0; i < l; i++) {
        if (infix[i] == '(') {
            infix[i] = ')';
        } else if (infix[i] == ')') {
            infix[i] = '(';
        }
    }

    string prefix = infixToPostfix(infix);  // Convert the modified infix to postfix

    // Reverse the postfix expression to get the prefix
    // For infix to prefix: You reverse at the end to get the correct prefix order.   
    // For infix to postfix: No reversal at the end is needed.
    reverse(prefix.begin(), prefix.end());

    return prefix;  // Return the prefix expression
}

int main() {
    string s = "(p+q)*(c-d^e^f)";  // Infix expression
    cout << "Infix expression: " << s << endl;
    cout << "Prefix Expression: " << infixToPrefix(s) << endl;  // Output the prefix expression
    return 0;
}
```
### Complexity Analysis:

- **Time Complexity:** O(N)  
  where N is the length of the infix expression. Each character is processed once.

- **Space Complexity:** O(N)  
  where N is the length of the infix expression. The stack can hold at most N characters in the worst case.


---

### Dry Run:

#### 1. Reverse the infix expression

| Step | Expression         | Note      |
|------|-------------------|-----------|
| 1    | (p+q)*(c-d^e^f)   | Original  |
| 2    | )f^e^d-c(*q+p(    | Reversed  |



#### 2. Swap '(' and ')'

| Step | Expression         | Note      |
|------|-------------------|-----------|
| 3    | (f^e^d-c)*(q+p)   | Swapped   |



#### 3. Convert to Postfix

We process: `(f^e^d-c)*(q+p)`

| i  | char | Stack      | Output      | Action/Note                        |
|----|------|------------|-------------|------------------------------------|
| 0  | (    | (          |             | push '('                           |
| 1  | (    | ((         |             | push '('                           |
| 2  | f    | ((         | f           | output operand                     |
| 3  | ^    | ((^        | f           | push '^'                           |
| 4  | e    | ((^        | fe          | output operand                     |
| 5  | ^    | ((^^       | fe          | push '^' (right-assoc)             |
| 6  | d    | ((^^       | fed         | output operand                     |
| 7  | -    | ((-        | fed^^       | pop '^', '^', push '-'             |
| 8  | c    | ((-        | fed^^c      | output operand                     |
| 9  | )    | (          | fed^^c-     | pop '-', pop '('                   |
| 10 | *    | (*         | fed^^c-     | push '*'                           |
| 11 | (    | (*(        | fed^^c-     | push '('                           |
| 12 | q    | (*(        | fed^^c-q    | output operand                     |
| 13 | +    | (*(+       | fed^^c-q    | push '+'                           |
| 14 | p    | (*(+       | fed^^c-qp   | output operand                     |
| 15 | )    | (*         | fed^^c-qp+  | pop '+', pop '('                   |
| 16 | )    |            | fed^^c-qp+* | pop '*', pop '('                   |

Final Output after stack is empty:  
`fed^^c-qp+*`



#### 4. Reverse Postfix to get Prefix

| Step | Expression      | Note         |
|------|----------------|--------------|
| 4    | fed^^c-qp+*    | Postfix      |
| 5    | *+pq-c^^def    | Prefix       |

---

#### Final Prefix Expression
  `*+pq-c^^def`


