...
// To implement a node in a doubly linked list that will store data items
struct Node {
    int key, value, cnt;
    Node *next;
    Node *prev;
...
private:
    ...
    // To store the frequency of the least frequently used data item
    int minFreq;
...
public:
    ...
    void updateFreqListMap(Node *node) {
        ...
        // Get the list for the next higher frequency (cnt + 1)

        // If it exists, use the existing list; otherwise, create a new one
        List *nextHigherFreqList;

        if (freqListMap.find(node->cnt + 1) != freqListMap.end()) {
            // Use the existing list for frequency cnt + 1
            nextHigherFreqList = freqListMap[node->cnt + 1];
        }
        else
        {
            // Create a new list for frequency cnt + 1
    ...
    void put(int key, int value) {
        // If the size of the cache is 0, no data-items can be inserted 
        if (maxSizeCache == 0) {
            return;
        }
        ...
        else
        {
            ...
            if (freqListMap.find(minFreq) != freqListMap.end()) {
                listFreq = freqListMap[minFreq];  // Use existing
            } else {
                  listFreq = new List();             // Create only if needed
                  freqListMap[minFreq] = listFreq;   // Store it
            }
