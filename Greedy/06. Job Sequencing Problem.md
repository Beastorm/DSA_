...
1. `(1, 4, 20)` // Mean you have to complete this task on or before job id 4 task
2. `(2, 1, 10)`
3. `(3, 1, 40)`
4. `(4, 1, 30)`
...
## Approach (DSU or Path Compression):
...
// DSU find function, Finds the latest available slot <= num
...
public:
    vector<int> JobScheduling(Job arr[], int n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (availableSlot > 0) {
                // Job can be scheduled at an available slot
...
- **Time Complexity:**
   - **Sorting:** `O(N log N)`
   - **DSU Operations:** We perform `N` find operations. With path compression, each takes `O(α(M))`, which is nearly constant ($\le 4$).
