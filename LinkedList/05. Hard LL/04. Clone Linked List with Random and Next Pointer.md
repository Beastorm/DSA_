...
<img src="https://static.takeuforward.org/content/1.png-UtgNZnhE" width="520">
...
Node* copyRandomList(Node* head) {
    if (!head) return NULL;
    unordered_map<Node*, Node*> mpp; // Map original node to its copy
    ...
    while (tmp) {
        ...
        copyNode->next = mpp[tmp->next];       // Set next pointer
        copyNode->random = mpp[tmp->random];   // Set random pointer
        tmp = tmp->next;
    }
    return mpp[head]; // Return head of copied list
}
