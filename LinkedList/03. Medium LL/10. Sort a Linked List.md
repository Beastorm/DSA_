...
3. Split the linked list into two halves at the midpoint by pointing the middle to null, where the left half starts from the head and the right half starts from the node after the middle.
...
public:
    ...
    Node* findMiddle(Node* head) {
        // If the list is empty or a single node
