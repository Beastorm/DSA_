...
- **Time Complexity:** $O(N)$ + $O(NlogN)$ + $O(N)$, where `N` is the total number of nodes in all lists.
...
- **Time Complexity:** $O(Nlogk)$  
  ...
  - Each heap operation (insert or extract-min) takes $Ologk)$ time.
  - Therefore, for `N` nodes, the total time is $O(Nlogk)$.
