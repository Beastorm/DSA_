...
**Problem Statement:**  
Given a string `s`, count the total number of **distinct palindromic subsequences** present in it.

A **palindromic subsequence** is a sequence derived by deleting zero or more characters (without changing the order) such that the resulting string reads the same forwards and backwards.
...
**Explanation:**  
The 4 distinct palindromic subsequences are:  
`"a"`, `"b"`, `"c"`, `"d"`.
...
All subsequences are generated (`2^n`), and each subsequence can take up to `O(n)` time to check if it is a palindrome.

**Space Complexity: `O(n)`**  
Due to recursion stack or temporary string storage.
...
### Steps
- Start with the full string and count palindromic subsequences between two positions `i` and `j`.
- If the part is empty (`i > j`), return `0` since there is no subsequence.
- If there is only one letter (`i == j`), return `1` since a single letter is always a palindrome.
- If the first and last letters of the part are the same (`s[i] == s[j]`):
  - Count palindromes in the middle (`i+1` to `j-1`)
  - Count palindromes excluding the first (`i+1` to `j`)
  - Count palindromes excluding the last (`i` to `j-1`)
  - Combine them and add one extra for the new pair
- If the first and last letters are different (`s[i] != s[j]`):
  - Count excluding the first (`i+1` to `j`)
  - Count excluding the last (`i` to `j-1`)
  - Subtract the overlap counted in both (`i+1` to `j-1`)
- Save each result in a DP table so that if the same range is needed again, return the saved answer directly.
...
Each substring state `(i, j)` is computed once and reused.

**Space Complexity: `O(n^2 + n)`**  
`O(n^2)` for the memoization table and `O(n)` for the recursion stack.
...
- For each part, check the first and last letters:
  - If they are the same, use the answers from inner parts and also add extra for the pair.
...
**Time Complexity: `O(n^2)`**  
All substrings of all lengths are processed once.

**Space Complexity: `O(n^2)`**  
A 2D DP table is used to store the results of subproblems.
