...
**Problem Statement:**  
Minimum Characters to Add at Front to Make String Palindrome
...
### Idea
We want the **longest prefix** of `s` that is already a palindrome.  
If a prefix `s[0..i]` is a palindrome, then we only need to add the reverse of the remaining suffix `s[i+1..n-1]` in front.  
To minimize added characters, we try the **largest palindromic prefix**.
...
3. If no prefix matches (won’t happen for non-empty strings because a single character is always a palindrome), return `n - 1`.
...
class Solution {
public:
    // Return minimum characters to add at front to make string palindrome
    int minChar(string str) {
        ...
        // lps[n-1] gives length of longest prefix of original string
