...
💡 **Example:**  
Text = `"hello"`, Pattern = `"ll"` → output = `2` (because `"ll"` starts at index `2`).
...
3. For each index `i` from `0` to `n - m`:
   ...
   - If `second == m`, it means the whole `needle` matched → return `i`.
...
- Outer loop tries all start positions: `n - m + 1` ≈ `O(n)`
...
4. While `first < haystack.length()`:
   - If `haystack[first] == needle[second]`:
     ...
     - if `second == needle.length()`, a full match is found → return `first - second`
   - Else (mismatch):
