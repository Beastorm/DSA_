...
> **Note:** **Rabin–Karp Fingerprint Function (Rolling Hash)**  
> A common Rabin–Karp fingerprint (hash) for a string `s` of length `m` is:  
...
> **Note:** We take modulo (`% q`) in Rabin–Karp hashing because:
...
> **Note:** A **spurious hit** (in Rabin–Karp) is a **false match** where the hash of the pattern equals the hash of a text window, but the actual strings are different (hash collision).  
> Rabin–Karp handles this by doing a **character-by-character verification** whenever the hashes match.

---
>[!NOTE]
>## Horner’s Method:

### Why `hash = hash*d + s[i]` Works ?
...
**2) Rewrite using Horner’s method (nested form):**  
...
public:
    vector<int> rabinKarp(const string& txt, const string& pat) {
        ...
        // Remove Old: Subtract the leading digit multiplied by the highest place value (532 - 5x100 = 32).
        // Shift & Add New: Multiply the remainder by the base to shift left, then add the incoming digit
        // (32x10 + 8 = 328).
        // Calculate h in advance, so that it can be used in removing leading char d^(m-1) % q
        long long h = 1; 
        for (int i = 0; i < m - 1; i++) {
            h = (h * d) % q;
        }
        ...
        // Initial hash computation using Horner’s Method Formula
        for (int i = 0; i < m; i++) {
