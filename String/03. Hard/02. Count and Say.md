...
> **Note:** The loop appends a group only when it detects a change (`result[j] != result[j-1]`).  
> The *last group* reaches the end of the string, so there is no next character to trigger a change.  
> Therefore, after the loop, we must manually append the final counted group.

> **Example:** `result = "11122"`  
> - A change is detected at `j = 3` (`'2' != '1'`), so `"31"` is appended for `"111"`.  
> - The remaining `"22"` is never appended inside the loop (no next change).  
> - So we add it after the loop:  
>   `current += to_string(count) + result.back();`  (here `count = 2`, `result.back() = '2'`)
...
public:
    ...
    string countAndSay(int n) {
        ...
        for (int i = 1; i < n; ++i) {
            ...
            // So starting from 1 is necessary to safely compare with j-1.
...
- **Time Complexity: `O(L1 + L2 + ... + Ln)`**  
    Each term is scanned once to build the next term. If `Ln` is the length of the `n`th term, the total time is the sum of the lengths across all generated terms. This is often described as **exponential growth** in `n`.
