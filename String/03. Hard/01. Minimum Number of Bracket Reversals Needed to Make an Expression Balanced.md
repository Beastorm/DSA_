...
I cannot guarantee every string with counts `open = 3`, `close = 1` is literally `")((("` (for example, `"((()"` isn’t).
...
That’s why earlier the case `open = 3`, `close = 1` corresponds to the reduced form `")((("`, even though `"((()"` itself needs only `1` reversal.  

Question: Why can't we pair with left over opening with left over closing?
No, because an unmatched opening and an unmatched closing (after the cancellation step) are in the wrong order, so they can’t pair as `()`.
...
- It needs `1` reversal:
  ...
  - Better: flip the second `(` to `)` → `"()"` (balanced)

So even though counts match (`1` open, `1` close), **order** makes it invalid, so you still need reversals.

That’s why we can’t just “pair 1 opening with 1 closing” unless they appear as `"()"` in the correct order.
