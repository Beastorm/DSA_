...
1. Create a `Z` array of size `n`, set `Z[0] = 0`.
...
>- **Z-box = [L, R]**: a window where `s[L..R]` matches the prefix `s[0..R-L]`.
>- **Outside Z-box**:
  >- Condition: `i > R`
  >- Meaning: no reliable precomputed match info for `i`
  >- Action: start fresh → set `L = R = i`, compare from scratch
  
>- **Immediate mismatch at i (`s[i] != s[0]`)**:
  >- Set `L = R = i`
  >- While-loop does not run
  >- `z[i] = 0`
  >- Then `R--` ⇒ `R = i - 1`
  >- So `R < L` ⇒ Z-box becomes **empty (size 0)** (no useful box)
...
s = "abcabxabc"
n = 9
Initialize: z = [0,0,0,0,0,0,0,0,0], L=0, R=0
-------------------------------------------------------

i=1: i>R (1>0) outside
  L=R=1
  compare s[0] vs s[1] : 'a' vs 'b' mismatch
  z[1]=0, R-- => R=0
  L=1, R=0

i=2: i>R (2>0) outside
  L=R=2
  compare s[0] vs s[2] : 'a' vs 'c' mismatch
  z[2]=0, R-- => R=1
  L=2, R=1
...
i=5: i>R (5>4) outside
  L=R=5
  compare s[0] vs s[5] : a!=x mismatch
  z[5]=0, R-- => R=4
  L=5, R=4
