...
- `suf` starts at `1` and only increases up to `n-1` → at most `n-1` increments.
...
- This “fallback chain” can happen multiple times.

Even then:
- Every fallback reduces `pre`, and total fallback reductions across the whole algorithm are bounded by how much `pre` can increase (≤ `n`).
