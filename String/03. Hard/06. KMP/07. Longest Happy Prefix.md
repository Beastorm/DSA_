...
### Intuition (Simple)
A “happy prefix” is a prefix that is also a suffix (but not the whole string).
We want the **longest** such prefix.

KMP’s **LPS array** tells for every position `i` the length of the longest prefix that matches a suffix ending at `i`.
