...
### Similar Problem:
Relation to Rotation Check Problem**  
...
## 🧠 KMP Approach
1. If `pattern.length() > text.length()`, no match is possible → return `[]`.
...
vector<int> circularMatches(const string& text, const string& pattern) {
    ...
    int first = 0; // index in doubled
    int second = 0; // index in pattern

    while (first < (int)doubled.size()) {
        if (doubled[first] == pattern[second]) {
            first++;
            second++;
            if (second == m) {
                int start = first - m; // match start in doubled

                // accept only starts within the original text range
