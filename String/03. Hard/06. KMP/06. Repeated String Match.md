...
public:
    int repeatedStringMatch(string a, string b) {
        ...
        // Sometimes need one extra repeat to cover overlap case
...
- Let `r = ceil(m / n)`. We build `temp` with about `r` repeats, so `temp` length is `O(n * r)` (≈ `O(m)`).  
- KMP search runs in linear time in the text length, so checking `temp` and `temp + a` costs `O(n * r)` each (overall still linear).

So overall time is **linear in the size of the built repeated string**, i.e. about `O(m + n)` in practice.
