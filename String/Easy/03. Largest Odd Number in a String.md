...
- **What is “largest odd” here?**

“Largest” means **numerically biggest**, not “contains the biggest odd digit”.

In this problem, the largest odd substring will always be the **longest possible prefix ending at the last odd digit** (because keeping more digits on the left makes the number bigger), and then you may need to **drop leading zeros**.


### What are NOT “largest odd” (common wrong picks)

- **Even-ending substrings are not valid odd numbers**  
  Example: `"...8"`, `"...0"`, `"...2"` → not odd.
...
- Leading zeroes don’t add value to the number, so remove them for a cleaner representation.
