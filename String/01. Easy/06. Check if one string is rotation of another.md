...
A rotation means you cut the string at some point and swap the two parts.

Example:  
`"rotation"` → cut after `"rota"` → `"tion" + "rota" = "tionrota"`

Trick:
- If you take `s + s`, it contains **all possible rotations** of `s` as substrings.
  - `"rotationrotation"` contains `"tionrota"`, `"ationrot"`, etc.

So:
1. If lengths are different → not a rotation.
2. Otherwise, check if `goal` is a substring of `s + s`.
...
class Solution {
public:
    // Check if goal is a rotation of s using the string doubling method
    bool rotateString(string& s, string& goal) {
        // Strings must be the same length to be rotations of each other
        if (s.length() != goal.length()) return false;
        
        // Concatenate s with itself and check if the goal exists in it
        string doubledS = s + s;
