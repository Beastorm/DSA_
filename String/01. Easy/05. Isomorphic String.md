...
**Trick to Remember:**  
**“Same pattern of repeats.”**  
Example:  
`paper` → p repeats at positions (0,2)  
`title` → t repeats at positions (0,2)  
So it works.
...
- If they are different → pattern breaks → not isomorphic.
...
### Algorithm:
- Think of an isomorphism as a consistent one-to-one mapping between characters of two strings.
...
✅ No mismatch occurs → strings are `isomorphic`.
...
| 2 | o    | r    | 2                | 0                | fail (2 != 0)| -               | -               |
 ...
 >At i = 2, o is repeating in s (seen before), but r is new in t, so the patterns don’t match → not isomorphic.



---

## 🧠 Approach - II (Isomorphic Strings using Hash Maps)
### Algorithm:
