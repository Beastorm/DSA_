    ...
    string reverseWords(string s) {
        ...
        for (int i = 0; i < s.size(); i++) {
            ...
            // If we encounter a space and have a word collected
                ...
                // Push the collected word to the words list
...
**Time Complexity: `O(N)`**  
We traverse the string once to collect words (`O(N)`) and once more to reverse and join them (`O(N)`). Hence, the total time is `O(N)`.
...
- While the pointer is within the string:
  ...
  - Move the pointer backward until a space or the start of the string is found.
  - Extract the word and append it to the result string.
  - If the result is not empty, add a space before appending the next word.
...
public:
    ...
    string reverseWords(string s) {
        ...
        while (i >= 0) {
            ...
            // Move left until a space or the start of the string is found
            while (i >= 0 && s[i] != ' ') {
                i--;
            }
