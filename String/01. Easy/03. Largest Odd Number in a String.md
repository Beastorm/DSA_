...
- **Substrings that start with `0` are not allowed as the returned number**  
  Example: in `"0214638"`, `"021463"` is odd but invalid due to the leading `0`.

- **A smaller odd substring when a bigger one exists**  
  Example: `s = "5347"`  
  Odd substrings include `"5"`, `"53"`, `"347"`, `"5347"`.  
  `"347"` is odd but not the largest because `"5347"` is bigger.
...
class Solution {
public:
    // Returns the largest odd-numbered substring from the given string
