    ...
    string removeOuterParentheses(string s) {
        ...
        for (char ch : s) {
            ...
            // If we encounter ')', decrease the level
                ...
                level--;
                // If we're inside a primitive, add ')' to the result
                if (level > 0) result += ch;
            }
        }
...
## 🧠 Approach -II: Stack-based Algorithm (Remove Outermost Parentheses)
...
### Why it works
- The first `'('` of each primitive is the one encountered when the stack is empty → we skip it.
- The last `')'` of each primitive makes the stack empty after popping → we skip it.  
