...
### Tiny Example
String: `"aa"`
- Substrings: `"a"`, `"a"`, `"aa"` → all have **1** distinct character
...
### right = 4, char = 's'
- freq = `{p:2, q:2, s:1}` (distinct=3 > 2) → shrink
  - left=0 ('p'): `{p:1,q:2,s:1}` distinct=3, left=1
  - left=1 ('q'): `{p:1,q:1,s:1}` distinct=3, left=2
  - left=2 ('p'): `{p:0,q:1,s:1}` remove p → `{q:1,s:1}` distinct=2, left=3
- valid window: `s[3..4] = "qs"`
- add `right-left+1 = 2`
- res = `12`

✅ `atMostKDistinct(s, 2) = 12`
...
### right = 1, char = 'q'
- freq = `{p:1,q:1}` distinct=2 > 1 → shrink
  - left=0 ('p'): `{p:0,q:1}` remove p → `{q:1}`, left=1
- add `1`
- res = `2`

### right = 2, char = 'p'
- freq = `{q:1,p:1}` distinct=2 > 1 → shrink
  - left=1 ('q'): `{q:0,p:1}` remove q → `{p:1}`, left=2
- add `1`
- res = `3`

### right = 3, char = 'q'
- freq = `{p:1,q:1}` distinct=2 > 1 → shrink
  - left=2 ('p'): `{p:0,q:1}` remove p → `{q:1}`, left=3
- add `1`
- res = `4`

### right = 4, char = 's'
- freq = `{q:1,s:1}` distinct=2 > 1 → shrink
  - left=3 ('q'): `{q:0,s:1}` remove q → `{s:1}`, left=4
- add `1`
- res = `5`

✅ `atMostKDistinct(s, 1) = 5`
