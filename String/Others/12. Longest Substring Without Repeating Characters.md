...
2. For every start index `i` from `0` to `n-1`:
   - For every end index `j` from `i` to `n-1`:
     1. Check if substring `s[i..j]` has all unique characters:
        ...
        - Scan `k` from `i` to `j`:
          - If `s[k]` is already marked in `seen`, then substring is invalid (has duplicates).
          - Otherwise mark it as seen.
     2. If substring is valid, update:
        - `best = max(best, j - i + 1)`
3. Return `best`.

## Why it is O(n^3)
- There are `O(n^2)` substrings (`i` and `j` loops).
- For each substring, the uniqueness check scans up to `O(n)` characters.
...
```
### Complexity Analysis (O(n^3) brute force)
...
### Intuition (O(n^2) brute improvement)
...
## Complexity Analysis
- Time: In the worst case (all characters unique), the inner loop runs ~`n-i` times for each `i`:
  - Total operations: `(n) + (n-1) + ... + 1 = n(n+1)/2 = O(n^2)`
  - Set operations are average `O(1)` with hashing.
  - Overall: **O(n^2)** average.
- Space: The set can hold at most `min(n, Σ)` characters (`Σ` = character set size), so:
  - **O(min(n, Σ))**, which is **O(1)** if Σ is fixed (e.g., ASCII).
...
// O(n^2) approach: for each start i, extend j until duplicate appears.
...
### Complexity Analysis (O(n^2) approach)
...
2. While `second < n`:
   ...
   5. Move right end forward: `second++`.
3. Return `bestLen`.
...
// Sliding window using count[256] like in the image: O(n)
