...
3. For every start index `i` from `0` to `n-1`:
   - For every end index `j` from `i` to `n-1`:
     1. Check if substring `str[i..j]` contains all `need` distinct characters:
        ...
        - Count how many distinct characters are marked; if it equals `need`, substring is valid.
...
// O(n^2): for each start i, expand j and maintain distinct count for str[i..j]
