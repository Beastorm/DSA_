...
To find the maximum falling path sum by starting from the **last row** and moving toward the **first row**, we change our perspective. Any cell $(i, j)$ in the current row can be reached from three cells in the row above it:
1. Directly above: $(i-1, j)$
2. Diagonal Left: $(i-1, j-1)$
3. Diagonal Right: $(i-1, j+1)$

We want to find the maximum path sum ending at **any** cell in the last row. To do this, we recursively look "up" to find the best path that leads to our current position.

---

### Algorithm:

1.  **Function Definition:** `maxPath(i, j)` returns the maximum path sum from the first row to cell $(i, j)$.
2.  **Base Case (First Row):** If $i == 0$, we have reached the top. Return the value of the cell itself: `matrix[0][j]`.
3.  **Boundary Conditions:** If $j < 0$ or $j >= m$, this is an invalid column. Return a very small number (e.g., `-1e9`) so `max()` ignores this path.
4.  **Recursive Steps (Looking Up):**
    * `up = maxPath(i - 1, j)`
    * `leftDiagonal = maxPath(i - 1, j - 1)`
    * `rightDiagonal = maxPath(i - 1, j + 1)`
5.  **Return:** `matrix[i][j] + max({up, leftDiagonal, rightDiagonal})`
...
// Recursive function to find max path sum ending at cell (i, j)
int maxPath(int i, int j, vector<vector<int>> &matrix, int m) {
    // 1. Boundary Check: If the column is out of bounds
    if (j < 0 || j >= m)
        return -1e9; // Return very small value to ignore this path

    // 2. Base Case: Reached the first row
    if (i == 0)
        return matrix[0][j];

    // 3. Recursive Calls: Look at the three possible cells in the row above
    int up = maxPath(i - 1, j, matrix, m);
    int leftDiagonal = maxPath(i - 1, j - 1, matrix, m);
    int rightDiagonal = maxPath(i - 1, j + 1, matrix, m);

    // 4. Return the current cell value + the best path sum from above
    return matrix[i][j] + max({up, leftDiagonal, rightDiagonal});
}
...
int maximumPathSum(vector<vector<int>> &matrix) {
    ...
    // The path can end at any cell in the last row
    // We check each cell in the last row (n-1) to find the absolute maximum
    for (int j = 0; j < m; j++) {
        res = max(res, maxPath(n - 1, j, matrix, m));
    }
...
int main() {
    ...
    cout << "Maximum falling path sum: " << maximumPathSum(matrix) << endl;
