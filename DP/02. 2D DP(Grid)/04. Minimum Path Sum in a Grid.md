...
- From any cell, we can reach it only from the **top** or the **left** cell.
- The minimum cost to reach a cell depends on the minimum cost to reach its top or left neighbor.
- A greedy approach fails because a locally cheaper choice may lead to a higher cost later.
- Hence, we explore both possibilities using recursion.
- To avoid recalculating the same subproblems, we use memoization.


### Algorithm:

1. Start recursion from the bottom-right cell.
2. If the current cell is the top-left cell, return its cost.
3. If the indices go out of bounds, return a very large value.
4. If the result for the current cell is already computed, return it.
5. Recursively calculate the minimum path sum from:
   - the top cell
   - the left cell
6. Add the current cell cost to the minimum of the two values and store it.
7. The value at the bottom-right cell is the final minimum path sum.
...
int findMinPathSum(int row, int col,
    ...
    // Out of bounds
    if (row < 0 || col < 0)
        return 1e9; // large value to ignore invalid paths
    ...
    // Recursive calls
    int fromUp = findMinPathSum(row - 1, col, grid, dp);
    int fromLeft = findMinPathSum(row, col - 1, grid, dp);

    // Store and return the minimum cost
    dp[row][col] = grid[row][col] + min(fromUp, fromLeft);
    return dp[row][col];
}
...
int minPathSum(vector<vector<int>> &grid) {
    ...
    return findMinPathSum(rows - 1, cols - 1, grid, dp);
}
