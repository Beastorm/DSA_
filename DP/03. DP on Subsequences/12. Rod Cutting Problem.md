...
Key observations:
- Each rod length can be used **multiple times** → this is an **Unbounded Knapsack** problem.
- For every possible rod length, we have **two choices**:
  1. **Do not cut** using this length → move to smaller lengths.
  2. **Cut** using this length → reduce remaining rod length and stay on the same index.
...
Given:
- `price[i]` → value of rod piece of length `i + 1`
- Rod length `N`
...
#### 1️⃣ Not Take the Current Rod Length
- Skip the current rod length
- Move to smaller rod sizes
notTake = f(ind - 1, N)


### 2️⃣ Take the Current Rod Length
...
A greedy approach fails because **locally optimal choices don’t guarantee a global optimum**.  
...
#### 1️⃣ Express the Problem in Terms of Indexes
- We are given:
  - `wt[]` → weights of items  
  - `val[]` → values of items  
  - `W` → knapsack capacity  
...
- Each rod piece length `i` (1-based) has:
  - **length = i**
  - **value = price[i-1]**
- You can cut the rod **any number of times** → items are **unlimited**
...
**Where:**
- `ind` → index in `price[]` (0-based)
- `len` → current rod length (0 to N)
...
- `dp[ind][len]` depends on:
  - **previous row** → `dp[ind-1][len]`
  - **same row** → `dp[ind][len-(ind+1)]`
...
- **Not take** → dp[len] (already present)
- **Take** → price[ind] + dp[len - (ind + 1)]

dp[len] = max(dp[len], price[ind] + dp[len - rodLength])


⚠️ Loop `len` **from left to right** (0 → N)  
