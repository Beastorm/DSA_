...
We simply need to check **existence** of at least one subset whose sum equals the target.  
...
2. **Pick the current element** (only if `arr[ind] <= target`)
