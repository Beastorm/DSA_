...
**Problem Statement:**   
Given an integer array `coins` representing coins of different denominations and an integer `amount` representing a total amount of money, return the **fewest number of coins** needed to make up that amount. If the amount cannot be made up, return `-1`. Assume infinite supply of each coin type.
...
**Output:** 3  
**Explanation:** 11 = 5 + 5 + 1 → 3 coins required.

### Example 2:
**Input:** coins = [2, 5], amount = 3  
**Output:** -1  
**Explanation:** It's impossible to make amount 3 with coins 2 and 5.
...
1. For each coin at index `i`, we have two choices:  
   - **Not Pick:** Skip the coin, move to next index.  
   - **Pick:** Use the coin, reduce target by coin value, and count 1 coin.  
