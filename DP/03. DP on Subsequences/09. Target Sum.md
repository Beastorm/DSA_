...
We recursively try both choices for each index and count how many ways lead to the required target.

### Recursive Definition:  

Let `f(ind, currSum)` be the number of ways to reach `Target`  
using elements from index `ind` to `N-1`, with current sum `currSum`.
...
### Recursive Case:  
- Add current element: `f(ind + 1, currSum + arr[ind])`
- Subtract current element: `f(ind + 1, currSum - arr[ind])`
