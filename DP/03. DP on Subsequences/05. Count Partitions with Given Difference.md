...
👉 **Count the number of subsets whose sum is equal to `(totalSum + D) / 2`.**   
...
- Many subproblems repeat → use **memoization** to store results.
- Each state is defined by `(index, remainingSum)`.

### Algorithm:
1. Compute `totalSum` of the array.
2. If `(totalSum + D)` is odd → return `0` (no valid partition).
3. Set `target = (totalSum + D) / 2`.
...
5. Choices:
   - **Not pick** current element → `f(ind-1, target)`
   - **Pick** current element (if `arr[ind] <= target`) → `f(ind-1, target-arr[ind])`
...
👉 **Count the number of subsets with sum = target**,  
where  
target = (d + totalSum) / 2

### Validity Checks
Before applying DP:
- If `(d + totalSum)` is **odd** → no valid partition
- If `d > totalSum` → no valid partition
...
3. Set `target = (totalSum + d) / 2`
4. Create a DP array `dp[0…target]`, initialize:
   - `dp[0] = 1` (empty subset)
...
Backward traversal ensures we don’t reuse the same element multiple times.
