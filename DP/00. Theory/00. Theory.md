...
* **Time Complexity:** $O(2^n)$
    * **Explanation:** Without memoization, the same values are recomputed many times in a recursive tree. This causes the number of calls to grow exponentially, leading to a time complexity of $O(2^n)$

* **Space Complexity:** $O(n)$
    * **Explanation:** The space complexity is determined by the maximum depth of the recursive call stack. In the worst case, the stack stores $n$ recursive calls simultaneously before starting to backtrack.
...
**Steps:** (From Recursive to Tabulation)   
...
So, is there a need to maintain a whole array for it? The answer is ‘No’.   
