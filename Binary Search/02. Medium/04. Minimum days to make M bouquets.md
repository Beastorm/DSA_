...
We need `m * k = 6` roses in *non-overlapping adjacent groups of 3*.


Check Day 7:

Roses bloomed by *day 7* (i.e., `arr[i] <= 7`):

- `arr = [7, 7, 7, 7, 13, 11, 12, 7]`.
- Bloomed (≤ 7): `[7, 7, 7, 7, -, -, -, 7]`

We now look for *non-overlapping* runs of length `k = 3`:  

- From index 0 to 3, we have a run of 4 bloomed roses → this gives `4 / 3 = 1` bouquet.
- At index 7, we have a single bloomed rose → not enough to form another bouquet.

By day 7, we can form only one bouquet, not two.  


Check Day 12:

Roses bloomed by *day 12*:

- Bloomed (≤ 12):  
  `arr = [7, 7, 7, 7, -, 11, 12, 7]`

Runs of consecutive bloomed roses:

- Run 1: indices `[0..3]` → length 4 → `4 / 3 = 1` bouquet
- Run 2: indices `[5..7]` → length 3 → `3 / 3 = 1` bouquet

Total = *2 bouquets*, which satisfies `m = 2`.

We must also check smaller days:

- On *day 11*, pattern is `[7,7,7,7, -, 11, -, 7]`  
  - Runs: length 4 (indices 0–3), then 1 (index 5), then 1 (index 7)  
  - Only *1* bouquet possible.

So the *minimum day* to make 2 non-overlapping bouquets of 3 roses each is:
...
-   **Time Complexity:** `O((max(arr[]) - min(arr[]) + 1) * N)`, where  
    `{max(arr[]) -> maximum element of the array, min(arr[]) -> minimum element of the array, N = size of the array}`.
