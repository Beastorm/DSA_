...
We want the *minimum ship capacity* `cap` such that all packages can be shipped within `d` days.

1. **Search range for capacity:**
   - The ship must be able to carry at least the heaviest package:
     ```text
     low  = max(weights)
     ```
   - In the worst case, one ship carries all packages in one day:
     ```text
     high = sum(weights)
     ```
   - So:
...
2. **How to check if a capacity `cap` is feasible:**
   - Load packages *in order*, day by day.
   - For each day, keep adding weights while the sum ≤ `cap`.
   - When adding the next package would exceed `cap`, start a *new day*.
   - Count how many days are needed.

3. **Feasibility condition:**
   - A capacity `cap` is *valid* if:
     ```text
     days_used <= d
     ```
   - If `cap` works (uses ≤ d days), we try a *smaller* capacity.
   - If `cap` doesn’t work (uses > d days), we need a *larger* capacity.

Because this “works/doesn’t work” is monotonic in `cap`, we can binary search on `[max(weights), sum(weights)]` to find the *minimum valid capacity*.
