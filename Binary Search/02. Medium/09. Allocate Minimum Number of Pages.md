...
- _Each student gets at least one book._
- _Each book should be allocated to only one student._
- _Book allocation should be in a contiguous manner._
- _You cannot leave any book unallocated._
- _No sharing or splitting of a book._
...
We must split the books (contiguously) into 2 groups so that the *maximum pages any student gets is as small as possible*.

All possible contiguous allocations:
...
Among these, the *minimum* possible "maximum pages" is `113`.
...
We need to divide 5 books among 4 students, contiguously.

A good allocation is:
...
The maximum pages any student gets here is *71*.

Can we do better?

Try another valid split:

- `[25] | [46] | [28, 49] | [24]`
  - Pages: 25, 46, 28+49=77, 24 → max = **77** (worse)

- `[25] | [46, 28] | [49] | [24]`
  - Pages: 25, 46+28=74, 49, 24 → max = **74** (still worse)

So the *best* we can do is maximum pages = `71`.
...
We have *3 books* and *4 students*.

Conditions say:

- Each student must get *at least one book*.
- Books cannot be split and must be allocated contiguously.

Since we have fewer books than students (`3 < 4`), it's *impossible* to give every student at least one book.
...
4.  Eventually, we land on the smallest value that works—this is our answer.
