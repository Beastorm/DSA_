...
public:
    ...
    long double minimiseMaxDistance(vector<int> &arr, int k) {
        ...
        priority_queue<pair<long double, int>> pq; // Pair of < section_distance, section_index >
        ...
        for (int gasStations = 1; gasStations <= k; gasStations++) {
            ...
            // The first time we push into the heap, we use the original length. 
            // But after that, the heap no longer stores original lengths.
...
public:
    // Function to calculate the number of gas stations required such that
