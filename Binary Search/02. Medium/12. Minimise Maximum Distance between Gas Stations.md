...
public:
    ...
    long double minimiseMaxDistance(vector<int> &arr, int k) {
        ...
        priority_queue<pair<long double, int>> pq; // Pair of distance and section index 
        ...
        for (int gasStations = 1; gasStations <= k; gasStations++) {
            ...
            // The first time we push into the heap, we use the original length. But after that, the heap no longer stores original lengths.
