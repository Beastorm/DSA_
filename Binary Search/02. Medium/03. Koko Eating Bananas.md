...
2. **Hours needed for a given `k`:**
   - For each pile `p`:
     ```text
     hours_for_pile = ceil(p / k)
     ```
   - Total hours:
     ```text
     total_hours = Σ ceil(a[i] / k)
     ```
...
4. **Monotonicity (for binary search):**
   - If a speed `k` works, any larger speed also works.
   - If a speed `k` fails, any lower speed also fails.
...
We need the **minimum** `k` such that Koko can finish all bananas in at most 8 hours.
...
So the **minimum** `k` that works is:
...
- `k = 22`  
  ...
  - 4  → `1`  
  - 20 → `1`  
  - **Total = 2 + 1 + 2 + 1 + 1 = 7` hours → too slow**

So the **minimum** `k` that lets Koko finish in 6 hours is:
...
Koko has 3 hours and 3 piles.

- If `k = 5`:
  - Each pile takes `ceil(5/5) = 1` hour  
  - **Total = 3` hours → works**

- If `k = 4`:
  - Each pile takes `ceil(5/4) = 2` hours  
  - **Total = 2 + 2 + 2 = 6` hours → too slow**
