...
### Conditions in the Binary Search Check (helper function)

When we binary search on a candidate value `mid` (possible answer), we use:

- Greedily form subarrays where each subarray has sum **≤ `mid`**.
- Count how many subarrays are needed.

Condition:

```text
If number_of_subarrays_needed <= K → mid is a valid (big enough) max sum.
If number_of_subarrays_needed  > K → mid is too small.
