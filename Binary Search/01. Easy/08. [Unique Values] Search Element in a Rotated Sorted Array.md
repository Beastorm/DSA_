    ...
    int search(vector<int>& nums, int target) {
        ...
        while (low <= high) {
            ...
            if (nums[low] <= nums[mid]) {
                ...
                // **Note:** Here we have differnt-different sorted range of numbers, so we can't blindly explore the desired sorted portion
                // That's why we should check if the target lies in the sorted range of numbers, or not
