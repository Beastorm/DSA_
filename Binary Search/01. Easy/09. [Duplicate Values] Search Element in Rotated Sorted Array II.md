...
Now, the algorithm gets stuck. It **cannot determine** which half is properly sorted and which half contains the "pivot" or rotation point. For example, the `1` in our array could be on the left side of `mid` or the right side.
...
#### The Approach to Solve This Ambiguity

When faced with this specific `arr[low] == arr[mid] == arr[high]` case, we cannot make an intelligent decision to cut the array in half. The safest move is to **shrink the search space** from both ends.

The special code block handles this:
```cpp
