...
Array `nums1` has a length of `m + n`, where the first `m` elements denote the elements of `nums1` and rest are `0`s whereas `nums2` has a length of `n`.
...
4.  If there are any remaining elements in `nums2` (i.e., `j >= 0`), then copy them into `nums1`. If any elements remain in `nums1`, they’re already in place, so no extra work is needed.
