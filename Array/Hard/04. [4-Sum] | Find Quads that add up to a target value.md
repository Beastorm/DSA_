...
### Algorithm
1.  Create a set to keep only unique groups of four numbers.
...
### Algorithm
**Intuition:**
To improve upon the brute force approach, we can reduce the number of nested loops from four to three. Similar to the 3 Sum problem, we can fix two numbers (`arr[i]` and `arr[j]`) and iterate for the third number (`arr[k]`). For the fourth number, instead of looping again, we calculate the specific value required to reach the target sum and check if it exists in a Hash Set.
...
- **Time Complexity:** $O(N^3 \times \log(M))$, where $N$ is the size of the array and $M$ is the number of unique quadruplets.
    - **Reason:** We are mainly using 3 nested loops. Inside the loops, we perform operations on the `set` data structure (inserting the unique quadruplets) which take logarithmic time complexity.
