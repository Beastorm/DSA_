...
So the 1st row, 1st column, and 4th column are set to 0.
...
public:
    ...
    void setZeroes(vector<vector<int>>& matrix) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                // If the element is zero, mark its row and column
                if (matrix[i][j] == 0) {
                    row[i] = 1;
                    col[j] = 1;
                }
...
3. **Mark rows and columns using the first row/column:**  
...
public:
    ...
    void setZeroes(vector<vector<int>>& matrix) {
        ...
        // Mark rows and columns in the first row/column
        for (int i = 1; i < m; i++) {
            for (int j = 1; j < n; j++) {
                if (matrix[i][j] == 0) {
                    matrix[i][0] = 0;
