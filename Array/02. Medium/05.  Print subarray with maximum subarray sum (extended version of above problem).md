## Kadane's Algorithm: Maximum Subarray Sum in an Array [*Similar to 04. Problem*] 
...
1. Start iterating through the array, maintaining a running `sum`.
...
public:
    ...
    int maxSubArray(vector<int>& nums) {
        ...
        for (int i = 0; i < nums.size(); i++) {
            ...
            // Update maxi and subarray indices if the current sum is greater
            if (sum > maxi) {
                maxi = sum;
                ansStart = start;
                ansEnd = i;
