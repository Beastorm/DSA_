...
* **Time Complexity:** $O(N^3)$  
  Where $N$ is the size of the array. We are using three nested loops here. Though not all are running for exactly $N$ times, the overall time complexity is approximately $O(N^3)$.
...
class Solution {
public:
    // Function to find count of subarrays with sum equal to k using prefix sums and a hashmap
    int subarraySum(vector<int>& arr, int k) {
        // Size of the array
        int n = arr.size();
        ...
        // To count subarrays starting from index 0 when prefixSum == k (remove becomes 0)
        prefixSumCount[0] = 1;
...
## Why do we do `count += prefixSumCount[remove]`?

### Main idea
- `prefixSum(i)` = sum of `arr[0..i]`
- Subarray `(j+1 .. i)` sum = `prefixSum(i) - prefixSum(j)`
- We want it equal to `k`:
...
If `remove` has occurred before, then **each occurrence** gives a valid starting index `j`.
That’s why we add:
- `count += prefixSumCount[remove]`

---

### Case A: `prefixSumCount[remove] = 0` (add **0**)
Meaning: we never saw that needed prefix sum before, so **no** subarray ending at `i` can have sum `k`.
...
- `prefixSum = 1+2+3 = 6`
- `remove = 6 - 7 = -1`
- `-1` is not in the map → `prefixSumCount[-1] = 0`

✅ add `0` (no subarray ends here with sum 7)

---

### Case B: `prefixSumCount[remove] = 1` (add **1**)
**Example**  
`arr = [1, 2, 1, 2, 1]`, `k = 3`  
...
- `prefixSum = 7`
- `remove = 7 - 3 = 4`
- prefix sum `4` occurred once (at `i=2`)

So **1** valid subarray ends at `i=4`:
- `(2+1..4) = (3..4) = [2,1]`
...
### Case C: `prefixSumCount[remove] > 1` (add **multiple**)
**Example**  
`arr = [1, -1, 1, -1, 1]`, `k = 0`  
...
- `prefixSum = 0`
- `remove = 0 - 0 = 0`
- prefix sum `0` occurred **2 times** so far:
  - initially (`prefixSumCount[0]=1`)
  - at `i=1`

So **2** valid subarrays end at `i=3`.
