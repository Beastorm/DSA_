...
### Main idea
- `prefixSum` = sum from start up to current index `i`
- We want the subarray sum `k` ending at `i`
...
So:
- `remove = prefixSum - k`
- If `remove` appeared before, then each occurrence is a valid `j`
- That’s why:
  - `count += prefixSumCount[remove]`
...
- `prefixSum = 7`
- `remove = 7 - 3 = 4`
- Prefix sum `4` was seen **once** (at `i=2`)

So there is **1** subarray ending at `i=4` with sum `3`:
- from `j+1 = 2+1 = 3` to `4` → `[2, 1]`
...
- `prefixSum = 0`
- `remove = 0 - 0 = 0`
- Prefix sum `0` was seen **2 times** so far:
  - once at the start (`prefixSumCount[0] = 1`)
  - once at `i = 1`

So there are **2** subarrays ending at `i=3` with sum `0`.
