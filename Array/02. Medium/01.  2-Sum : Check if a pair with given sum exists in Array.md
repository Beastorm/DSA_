## 2-Sum: Check if a pair with the given sum exists in the array
...
**Output:** `NO`  
**Explanation:** There exist no two numbers whose sum is equal to the target.
...
4. If the loop finishes and no pair is found:
...
class Solution {
public:
    // Variant 1: Check if two numbers sum to the target using hashing
    string twoSumExists(vector<int>& arr, int target) {
...
* **Space Complexity:** $O(N)$  
  In the worst case, we may store all elements of the array in the hash map.
...
class Solution {
public:
    // Variant 1: Check if two numbers sum to the target using a two-pointer approach
    string twoSumExists(vector<int> arr, int target) {
        int n = arr.size();
    ...
    // Variant 2: Return original indices of two numbers that sum to the target
    vector<int> twoSumIndices(vector<int> arr, int target) {
        int n = arr.size();
        vector<pair<int, int>> numsWithIndex;
        for (int i = 0; i < n; i++) {
