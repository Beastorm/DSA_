...
2. Use an **inner loop** from `i+1` to end to check if any element to the right is **greater than or equal** to `nums[i]`.
3. If such an element is found, `nums[i]` is **not** a leader.
4. If no element to the right is greater or equal, then `nums[i]` is a **leader** and is added to the answer list.
