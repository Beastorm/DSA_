...
4. At each index `i`:
   ...
   - If `sum` becomes greater than the previous maximum `maxi`, update:
     ...
     - `ansStart = start`
     - `ansEnd = i`
   - If `sum` becomes negative, reset `sum = 0` (discard this subarray and start fresh from next index).
5. After the loop ends, `ansStart` and `ansEnd` indicate the subarray with maximum sum.
