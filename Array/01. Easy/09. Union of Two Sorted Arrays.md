...
We aim to find the common elements in `arr1` and `arr2`, and the distinct elements of `arr1` and `arr2`. Use a **Single map** to find the frequencies of elements in `arr1` and `arr2`.

As we are using only a single map, the common elements in `arr1` and `arr2` are treated as a single element for finding frequency, so there would be no duplicates.
...
5. Iterate over each key-value pair in `freq` (keys will be sorted because the map is ordered).
...
public:
    ...
    vector<int> FindUnion(int arr1[], int arr2[], int n, int m) {
        ...
        for (int i = 0; i < n; i++)
            freq[arr1[i]]++;
        // Loop through the second array and store frequency
        for (int i = 0; i < m; i++)
            freq[arr2[i]]++;
...
int main() {
    ...
    // Print all elements of the union
    for (auto &val : Union)
        cout << val << " ";
    // Return success
    return 0;
...
class Solution {
public:
    // Function to find the union of two arrays using a set
    vector<int> findUnion(int arr1[], int arr2[], int n, int m) {
...
2.  While neither pointer has reached the end:
    *   If the element pointed by the first pointer is smaller, add it to the result if not a duplicate, move the first pointer.
    *   If the element pointed by the second pointer is smaller, add it to the result if not a duplicate, move the second pointer.
    *   If both elements are equal, add one to the result if not a duplicate, move both pointers.
3.  After exiting the loop, append the remaining elements from either array, skipping duplicates.
...
public:
    ...
    vector<int> findUnion(int arr1[], int arr2[], int n, int m) {
        ...
        while (i < n && j < m) {
            ...
            if (arr1[i] < arr2[j]) {
                // Add element if the union is empty or different from the last added
...
*   **Time Complexity:** $O(m+n)$
    Because at max `i` runs for `n` times and `j` runs for `m` times. This happens when there are no common elements in `arr1` and `arr2`, and all elements in `arr1` and `arr2` are distinct.

*   **Space Complexity:** $O(m+n)$
    If the Space of Union ArrayList is considered.
