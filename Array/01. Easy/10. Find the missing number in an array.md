...
### Algorithm: 
For each number between `1` to `N`, we will try to find it in the given array using linear search. If we don’t find any of them, we will return the number.
...
4.  Initially, the flag value will be set to 0. While iterating the array, if we find the element, we will set the flag to 1 and break out of the loop.
...
## Optimized Approach - I (Summation Method)
...
## Optimized Approach II: XOR Method
