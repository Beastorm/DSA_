...
*Trick:* `(i,j)` `i` denotes the number of rows passed and the number of columns in each row. After that, just add the current column.
...
### 4) How does the computer interpret `arr[i]`?
`arr[i] = base_address + index * sizeof(element)`

**For a 2D array (row-major):**  
Convert `(row, col)` to 1D index:

`index = row * colCount + col`

So:
`arr[row][col] = base_address + (row * colCount + col) * sizeof(element)`

**Example:**  
- base address = `500`
- element size = `4`
- for `arr[2][1]` with `colCount = 3`:

`index = 2*3 + 1 = 7`  
`address = 500 + 7*4 = 528`

### 5) Why we must pass column size in C/C++ 2D arrays
...
## 2. Search an Element in a 2D Array:
...
---
## 5. Sum of Diagonal Elements (Square Matrix Only):
