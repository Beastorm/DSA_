...
```
### Complexity Analysis (Row-major Binary Search)
...
- Check every element (2 loops)
- Time: `O(N*M)`
- Space: `O(1)`

### Better approach (Staircase Search / Top-right method)
- Start from the top-right corner: `i = 0`, `j = M-1`
- While `i < N` and `j >= 0`:
  - If `mat[i][j] == x` → found
  - If `mat[i][j] < x` → move down (`i++`)
    - (Everything to the left is smaller, so left won’t help)
  - If `mat[i][j] > x` → move left (`j--`)
    - (everything below is bigger, so down won’t help)
...
Then comparing with `x` gives a clear decision every time, and we can discard a whole row/column.

### Why is the top-right chosen?
