...
**Problem Statement:**  
Given an array of `N` integers, your task is to find unique triplets that add up to give a sum of `0`. In short, you need to return an array of all the unique triplets `[arr[a], arr[b], arr[c]]` such that `i!=j, j!=k, k!=i`, and their sum is equal to `0`. *Pre-requisite is 2-Sum Problem*
