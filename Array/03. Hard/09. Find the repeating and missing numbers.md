...
public:
    ...
    vector<int> findMissingRepeatingNumbers(vector<int>& nums) {
        ...
        // arrays are 0-indexed, but your problem values are 1-based.
...
├──────────────────────────────────────────────────────┤
│  THE DIVISION:                                       │
│    **Note: We overwrite `val2` to save memory        │
│    val2 = val2 / val1                                │
│                                                      │
│         = (X + Y)(X - Y)                             │
│           ────────────────                           │
...
public:
    ...
    vector<int> findMissingRepeatingNumbers(vector<int>& nums) {
        ...
        // so here we r just re using the val2 rather than have new variable val3
...
#### Logic:
     The rightmost set bit we extract represents a specific bit position where the *missing* number and the *repeating* number differ (i.e., one number has a `0` at this position, and the other has a `1`).   
...
public:
    ...
    vector<int> findMissingRepeatingNumbers(vector<int>& nums) {
        ...
        // Group numbers from 1 to n based on the differentiating bit
        for (int i = 1; i <= n; i++) {
            
            // Check if `i` belongs to the 1 group based on the differentiating bit
            if ((i & number) != 0) {
