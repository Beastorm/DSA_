...
3.  While `left < n` and `right < m`:
    *   If `arr1[left] ≤ arr2[right]`, copy `arr1[left]` to `arr3[index]` and increment `left`.
...
## 🧠 Optimized Approach-I : O(1) Space Optimized 
...
##  🧠 Optimized Approach-II : O(1) Space Optimized (Gap Method)
...
4.  *Traverse & Swap:* Loop while `right < len`:
    *   *Identify Array:* Check if pointers fall in `arr1` (index `< n`) or `arr2` (index `≥ n`).
    ...
    void merge(long long arr1[], long long arr2[], int n, int m) {
        ...
        while (gap > 0) {
            ...
            while (right < len) {
                ...
                if (left < n && right >= n) {
                    swapIfGreater(arr1, arr2, left, right - n); // Used right - n, so that we can make 0-based indexing like right should point 0,1 indexes, etc. of arr2.
                }
