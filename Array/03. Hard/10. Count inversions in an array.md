...
such that A[j] < A[i].  
Example: 5 has index 0 and 3 has index 2 now (5,3) pair is inversion as 0 < 2 and 5 > 3 which will satisfy out conditions and for reverse sorted array we will get maximum inversions, and that is (n)*(n-1) / 2. For the above given array, there are 4 + 3 + 2 + 1 = 10 inversions.
...
│                                                             │
│  KEY INSIGHT:                                               │
│  • Counts "out of order" pairs                              │
│  • Measures distance from sorted state                      │
│  • 0 inversions = Sorted                                    │
