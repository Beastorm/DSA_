...
**Output:** `120`  
**Explanation:**   
In the given array, `1×2×3×4×5` gives the maximum product value.  
...
**1) Why does a normally running product fail?**  
- With sums (Kadane), if the sum becomes bad, you reset.
- With products, a negative product can later become a big positive if another negative appears.
- So you *can’t* reset just because the product is negative.

**2) What actually matters?**  
- The maximum product subarray usually lies inside a part of the array with *no zeros* (because `0` breaks the product).
- In a zero-free segment:
  - If negatives are *even* → product of the whole segment can be maximum.
  - If negatives are *odd* → total product is negative, so to get the best positive product, you must skip *one negative*, which effectively means:
    - ignore the part from start up to the *first negative*, OR
    - ignore the part from the *last negative* to the end.

**3) How prefix + suffix captures both choices?**  
- `pre` (left-to-right) explores products that may skip some left part.
- `suff` (right-to-left) explores products that may skip some right part.
- Taking `max(pre, suff)` at each step covers both “skip-left” and “skip-right” possibilities.

**4) Why reset at zero?**  
- If `pre` or `suff` becomes `0`, it means a zero was multiplied in.
- Any subarray crossing that zero is broken, so we restart the product after it.
- That’s why we set the product back to `1`.

✅ So scanning from both ends + resetting at zeros gives the answer in `O(n)` time.
...
1.  Traverse the array from left to right (prefix) to build the cumulative product.
...
### <ins>Notes:</ins> 
Notes: Can `pre` and `suff` be negative?
- Yes, `pre` and `suff` *can be negative*.
- They are running products, so multiplying by a negative number can make them negative.
- We *do not reset* when they become negative.
- We reset only when they become `0`, because `0` breaks the product chain.

**Example:**  
`arr = [-2, 3]`.
- `pre = 1`
- after `-2`: `pre = -2` (negative)
- after `3`: `pre = -6` (negative)
...
2.  For each number in the array starting from the second:
    ...
    *   If the product of the current number and the previous max is larger than the number itself, update current max to that product; otherwise, set it to the number (starting a new subarray).
    *   Similarly, if the product of the current number and the previous min is smaller, update the current min; otherwise, set it to the number.
3.  If the current max is greater than the answer so far, update the answer.
4.  Return the answer at the end.

```
┌──────────────────────────────────────────────────────────────────┐
│ Question: Why Swap When Current is Negative?                     |
|──────────────────────────────────────────────────────────────────|
| Answer: Multiplying by negative FLIPS the order:                 │     
│ The  largest becomes smallest, and the smallest becomes largest. │
│ So we SWAP before multiplying to keep the correct track.         │
└──────────────────────────────────────────────────────────────────┘
┌──────────────────────────────────────────────────────────────────┐
│  Zero resets both maxProd and minProd to 0.                      │
│  Acts as a BARRIER - subarray must start fresh.                  │
│  Handled AUTOMATICALLY by the algorithm!                         │
└──────────────────────────────────────────────────────────────────┘
```   
...
public:
    ...
    // of any contiguous subarray using an optimized approach
    int maxProduct(vector<int>& nums) {
