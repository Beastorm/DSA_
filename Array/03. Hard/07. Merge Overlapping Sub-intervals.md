...
- **Time Complexity**  
    ...
    2. Merging scan:
       - The outer index `i` jumps forward to `j`.
       - The inner `while` advances `j` from `0` to `n` overall (each interval processed once)
       - So merging part is `O(n)`.

    Total time:
    - *`O(n log n) + O(n) = O(n log n)`*

- **Space Complexity**  
    - Output array `ans` can store up to `n` intervals → `O(n)`.
    - Sorting may use extra stack/temporary space depending on implementation → typically `O(log n)` auxiliary
    
    Overall:
    - *Space = `O(n)`* (dominant because of output)
...
- **Time Complexity**  
    1. Sorting intervals: `O(n log n)`
    2. Single pass to merge:
       - Each interval is processed once
       - `merged.back()` operations are `O(1)`
       - Total merging work: `O(n)`.
    
    Total time:
    - *`O(n log n)`*

- **Space Complexity**  
    - Output `merged` can store up to `n` intervals: `O(n)`.
    - Sorting uses extra space depending on implementation (often `O(log n)` auxiliary)
    
    Overall:
    - *Space = `O(n)`* (dominant due to output)
