...
Reverse Pairs are those pairs where `i < j` and `arr[i] > 2*arr[j]`, meaning `a[i]` is more than doubled than `a[j]`.
...
``` text
┌─────────────────────────────────────────────────────────┐
│  Inversions detect ANY out-of-order pair                │
│  Reverse Pairs detect SIGNIFICANT anomalies             │
│                                                         │
│  Use Case: Finding drastic drops/changes in data        │
└─────────────────────────────────────────────────────────┘
```
---
...
>**Notes:**  
 >- In merge sort, the *actual sorting happens in `merge()`*.
 >- `mergeSort()` only *splits* the array into smaller halves until they are size `1` (already sorted).
 >- `merge()` *combines* two sorted halves into one sorted array by comparing elements and placing them in order.
 >- So: splitting happens in `mergeSort()`, and ordering happens during merging in `merge()`.
