## Lecture 13: Print Unique Subsets and Variations
...
- **Formula:**  
  ...
  (Every subset can be represented as a subsequence by preserving order; includes the empty subsequence)
...
- **Unique Subsets:**  
  If the array has duplicates, only print each subset once:
  - `{}`, `{1}`, `{2}`, `{1,2}`, `{1,2,2}`

- **All Subsequences:**
  - `""`, `"1"`, `"2"`, `"12"`, `"22"`, `"122"`, `"2"`, `"12"`, `"22"`, `"122"`
...
- When the input string is empty, you insert the current output string into a set (to ensure uniqueness and skip empty subsequence).
...
- Remove duplicates if you want only unique results.
