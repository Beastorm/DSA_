...
- **Choices:** Swap current index with every index ≥ current, skipping duplicates.
...
void solve(string& s, int start, vector<string>& v) {
    ...
    for (int i = start; i < s.size(); i++) {
        // If this character hasn't been swapped into 'start' position yet
        if (mp.find(s[i]) == mp.end()) {
            mp.insert(s[i]);        // Mark as used
            swap(s[start], s[i]);   // Swap current char into position
            
            solve(s, start + 1, v); // Recurse for the next position
            
            swap(s[start], s[i]);   // Backtrack (undo swap) to restore original string
        }
    }
}
...
vector<string> findPermutations(string S) {
    ...
    // Sort input to process duplicates together (good practice)
    sort(S.begin(), S.end());
