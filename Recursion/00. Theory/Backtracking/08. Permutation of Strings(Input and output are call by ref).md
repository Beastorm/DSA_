...
void generatePermutationsRecursive(string& inputString, int currentIndex, vector<string>& permutationsList) {
    ...
    for (int swapIndex = currentIndex; swapIndex < inputString.size(); swapIndex++) {
        ...
        // If this character hasn't been swapped into the current position yet,
        if (usedCharacters.find(currentChar) == usedCharacters.end()) {

            // Mark as used
            usedCharacters.insert(currentChar);                 

            // Swap current char into position
            swap(inputString[currentIndex], inputString[swapIndex]);   

            // Recurse for next position
            generatePermutationsRecursive(inputString, currentIndex + 1, permutationsList); 
