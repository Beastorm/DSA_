...
## Optimal Approach
### Intuition:
**1. The Core Idea: Reducing Work**
If you want to calculate $2^{100}$, you don't actually need to perform 100 multiplications.
* You could calculate $2^{50} \times 2^{50}$.
* But even better, $2^{100}$ is mathematically identical to $(2 \times 2)^{50}$, which is $4^{50}$.
* Notice that the **exponent is now half the size**, but the result remains the same!

**2. The Even vs. Odd Strategy**
The code uses a clever recursive trick to shrink the exponent $n$ until it reaches $0$ or $1$:

* **If $n$ is EVEN:** Use the property $x^n = (x^2)^{n/2}$.
    * *Example:* $2^8 \rightarrow (2 \times 2)^4 \rightarrow 4^4$. 
    * We reduced the problem from 8 steps to 4 in a single move.
* **If $n$ is ODD:** You can't divide it evenly, so you "pull out" one $x$ to make the remaining power even.
    Use the property $x^n = x \times x^{n-1}$.
    * *Example:* $2^9 \rightarrow 2 \times 2^8$. 
    * Now that $2^8$ is even, you can apply the **Even Strategy**.
