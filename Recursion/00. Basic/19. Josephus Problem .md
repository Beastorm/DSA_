...
- **Time Complexity:**  
  - We run the `solve` function **N-1** times (to remove N-1 elements).
  - Inside each call, `v.erase()` takes **O(N)** time to shift elements.
  - Total = (N calls) × (O(N) per call) = **O(N^2)**.

- **Space Complexity:**  
  - **Vector:** Stores N elements → O(N).
  - **Recursion Stack:** Depth is N → O(N).
  - Total = O(N) + O(N) = **O(N)**.
