...
public:
    ...
    void generateSubsetsRecursive(int currentIndex, 
        ...
        currentSubset.push_back(inputNumbers[currentIndex]);
        generateSubsetsRecursive(currentIndex + 1, inputNumbers, currentSubset, uniqueSubsets);
    ...
    vector<vector<int>> findAllUniqueSubsets(vector<int>& inputNumbers) {
        set<vector<int>> uniqueSubsets;
        vector<int> currentSubset;
        ...
        generateSubsetsRecursive(0, inputNumbers, currentSubset, uniqueSubsets);

        // Convert the set of unique subsets to a vector for the final answer
        vector<vector<int>> allUniqueSubsets(uniqueSubsets.begin(), uniqueSubsets.end());
...
int main() {
    UniqueSubsetFinder finder;
    
    vector<int> inputNumbers = {1, 2, 2};
    vector<vector<int>> resultSubsets = finder.findAllUniqueSubsets(inputNumbers);
    
    // Print the result
    cout << "All unique subsets: " << endl;
    cout << "[ ";
    for (const auto& subset : resultSubsets) {
        cout << "[ ";
...
public:
    // Function to generate all unique subsets using backtracking
    void generateSubsetsRecursive(int startIndex, 
                                   vector<int>& inputNumbers, 
                                   vector<int>& currentSubset, 
                                   vector<vector<int>>& allUniqueSubsets) {
        // Add the current subset to the result
        allUniqueSubsets.push_back(currentSubset);

        // Iterate over the array from startIndex onwards
        for (int currentIndex = startIndex; currentIndex < inputNumbers.size(); currentIndex++) {
            // Skip duplicates: if current number is same as previous at same recursion level
            if (currentIndex > startIndex && inputNumbers[currentIndex] == inputNumbers[currentIndex - 1]) {
                continue;
            }

            // Include inputNumbers[currentIndex] in current subset
            currentSubset.push_back(inputNumbers[currentIndex]);

            // Recurse for next index
            generateSubsetsRecursive(currentIndex + 1, inputNumbers, currentSubset, allUniqueSubsets);
    ...
    vector<vector<int>> findAllUniqueSubsets(vector<int>& inputNumbers) {
        ...
        generateSubsetsRecursive(0, inputNumbers, currentSubset, allUniqueSubsets);
...
int main() {
    ...
    vector<vector<int>> uniqueSubsets = generator.findAllUniqueSubsets(inputNumbers);
