...
## Combination Sum I - For Loop Variant
...
public:
    ...
    void findCombinationsRecursive(int currentIndex, int remainingTarget, 
        ...
        // Try all elements starting from currentIndex
        for (int nextIndex = currentIndex; nextIndex < candidates.size(); nextIndex++) {
            // Only pick if the element doesn't exceed the remaining target
            if (candidates[nextIndex] <= remainingTarget) {
                
                // Add the current element to the combination
                currentCombination.push_back(candidates[nextIndex]);

                // Continue with the SAME index to allow repeated elements
                findCombinationsRecursive(nextIndex,  // Same index (allows repetition)
                                           remainingTarget - candidates[nextIndex], 
                                           candidates, 
                                           allCombinations, 
                                           currentCombination);
