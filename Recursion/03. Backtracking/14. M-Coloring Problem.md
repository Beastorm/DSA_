...
- For each vertex, you try every color from 1 to M, check if it’s safe (no adjacent vertex has the same color), and move to the next vertex.
...
- **Space Complexity:** $O(N)$, for the recursion stack and the color assignment array.
