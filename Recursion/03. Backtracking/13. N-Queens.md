...
// Backtracking function to solve N-Queens
void solve(int row, int n, vector<string>& board, vector<vector<string>>& res, vector<bool>& cols, vector<bool>& diag1, vector<bool>& diag2) {
    if (row == n) {
...
- **Space Complexity:** $O(N^2)$ for storing the board and $O(N)$ for the recursion stack and boolean arrays.
  - $O(K * N^2)$ if you store all K solutions (each is an N×N board).
