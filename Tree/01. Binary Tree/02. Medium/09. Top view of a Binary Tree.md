...
We use a map data structure to store the nodes corresponding to each vertical level of the tree, as the map automatically sorts the elements based on their ascending value. Against each vertical level, the node highest in the tree at that vertical level is added by traversing the tree level-order (BFS).
...
![Alt text](https://static.takeuforward.org/content/-isIohPVt)
