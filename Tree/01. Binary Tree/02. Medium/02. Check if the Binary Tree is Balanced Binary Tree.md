...
- **Time Complexity:** $O(N²)$, where N is the number of nodes in the binary tree. For each node, we calculate the height of its left and right subtrees, and the height calculation takes $O(N)$ in the worst case, leading to an overall $O(N × N)$ = $O(N²)$.
